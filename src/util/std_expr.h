...
public:
  ...
  DEPRECATED("use unary_exprt(id, op) instead")
  explicit unary_exprt(const irep_idt &_id) : expr_protectedt(_id, type())
  {
    operands().resize(1);
  }
...
public:
  ...
  extractbits_exprt() : expr_protectedt(ID_extractbits, type())
  {
    operands().resize(3);
  }
...
public:
  ...
  with_exprt() : expr_protectedt(ID_with, type())
  {
    operands().resize(3);
  }
